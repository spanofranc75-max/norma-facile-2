...
async def quality_hub_summary(user: dict = Depends(require_role("admin", "ufficio_tecnico"))):
    ...
    # ── Welders ──
    welders_all = await db.welders.find({"user_id": user["user_id"], "tenant_id": tenant_match(user)}, {"_id": 0, "name": 1, "stamp_id": 1, "qualifications": 1}).to_list(200)
    expiring_patents = []
    expired_patents = []
    ...
    # ── Instruments ──
    instruments_all = await db.instruments.find({"user_id": user["user_id"], "tenant_id": tenant_match(user)}, {"_id": 0, "name": 1, "serial_number": 1, "next_calibration_date": 1, "instrument_type": 1, "status": 1}).to_list(200)
    expired_instruments = []
    ...
    open_ncs = await db.non_conformities.find(
        {"status": {"$ne": "chiusa"}, "user_id": user["user_id"], "tenant_id": tenant_match(user)}, {"_id": 0, "nc_id": 1, "nc_number": 1, "description": 1, "priority": 1, "status": 1, "date": 1, "source": 1}
    ...
    # ── Audits ──
    all_audits = await db.audits.find({"user_id": user["user_id"], "tenant_id": tenant_match(user)}, {"_id": 0, "audit_id": 1, "date": 1, "next_audit_date": 1, "audit_type": 1, "auditor_name": 1, "outcome": 1}).to_list(500)
    next_audit = None
    ...
    # ── Documents ──
    doc_count = await db.company_docs.count_documents({"user_id": user["user_id"], "tenant_id": tenant_match(user)})
