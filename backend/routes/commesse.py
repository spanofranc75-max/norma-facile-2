...
async def get_board_view(user: dict = Depends(require_role("admin", "amministrazione", "ufficio_tecnico", "officina"))):
    ...
    uid = user["user_id"]
    tid = user["tenant_id"]
    items = await db[COLLECTION].find({"user_id": uid, "tenant_id": tenant_match(user)}, {"_id": 0, "eventi": 0}).sort("updated_at", -1).to_list(500)
...
async def delete_commessa(commessa_id: str, user: dict = Depends(require_role("admin", "amministrazione", "ufficio_tecnico", "officina"))):
    ...
    if not doc:
        raise HTTPException(404, "Commessa non trovata")
    result = await db[COLLECTION].delete_one({"commessa_id": commessa_id, "user_id": user["user_id"], "tenant_id": tenant_match(user)})
    ...
    if prev_id:
        await db.preventivi.update_one(
            {"preventivo_id": prev_id, "user_id": user["user_id"], "tenant_id": tenant_match(user)},
...
async def emit_event(commessa_id: str, req: EventoRequest, user: dict = Depends(require_role("admin", "amministrazione", "ufficio_tecnico", "officina"))):
    ...
    uid = user["user_id"]
    tid = user["tenant_id"]
    doc = await db[COLLECTION].find_one({"commessa_id": commessa_id, "user_id": uid, "tenant_id": tenant_match(user)})
    if not doc:
        raise HTTPException(404, "Commessa non trovata")
...
async def get_commessa_hub(commessa_id: str, user: dict = Depends(require_role("admin", "amministrazione", "ufficio_tecnico", "officina"))):
    ...
    uid = user["user_id"]
    tid = user["tenant_id"]
    doc = await db[COLLECTION].find_one({"commessa_id": commessa_id, "user_id": uid, "tenant_id": tenant_match(user)}, {"_id": 0})
    if not doc:
        raise HTTPException(404, "Commessa non trovata")
    ...
    if moduli.get("rilievo_id"):
        r = await db.rilievi.find_one({"rilievo_id": moduli["rilievo_id"], "user_id": uid, "tenant_id": tenant_match(user)}, proj)
        if r:
            hub["moduli_dettaglio"]["rilievo"] = r

    if moduli.get("distinta_id"):
        d = await db.distinte.find_one({"distinta_id": moduli["distinta_id"], "user_id": uid, "tenant_id": tenant_match(user)}, proj)
        if d:
            hub["moduli_dettaglio"]["distinta"] = d

    if moduli.get("preventivo_id"):
        p = await db.preventivi.find_one({"preventivo_id": moduli["preventivo_id"], "user_id": uid, "tenant_id": tenant_match(user)}, proj)
        if p:
            hub["moduli_dettaglio"]["preventivo"] = p

    if moduli.get("fatture_ids"):
        fatture = await db.invoices.find(
            {"invoice_id": {"$in": moduli["fatture_ids"]}, "user_id": uid, "tenant_id": tenant_match(user)}, proj
        ).to_list(50)
        hub["moduli_dettaglio"]["fatture"] = fatture

    if moduli.get("ddt_ids"):
        ddts = await db.ddt_documents.find(
            {"ddt_id": {"$in": moduli["ddt_ids"]}, "user_id": uid, "tenant_id": tenant_match(user)}, proj
        ).to_list(50)
        hub["moduli_dettaglio"]["ddt"] = ddts

    if moduli.get("fpc_project_id"):
        fpc = await db.fpc_projects.find_one({"project_id": moduli["fpc_project_id"], "user_id": uid, "tenant_id": tenant_match(user)}, proj_light)
        if fpc:
            hub["moduli_dettaglio"]["fpc_project"] = fpc

    if moduli.get("certificazione_id"):
        cert = await db.certificazioni.find_one({"cert_id": moduli["certificazione_id"], "user_id": uid, "tenant_id": tenant_match(user)}, proj)
        if cert:
