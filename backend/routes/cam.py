...
async def calcola_cam_per_commessa(commessa_id: str, user: dict = Depends(get_current_user)):
    ...
    await db.calcoli_cam.update_one(
        {"commessa_id": commessa_id, "user_id": user["user_id"], "tenant_id": tenant_match(user)},
        {"$set": {**risultato, "user_id": user["user_id"], "tenant_id": tenant_match(user)}},
        upsert=True
    )
...
async def import_cam_da_certificato(
    ...
    doc = await db.commessa_documents.find_one(
        {"doc_id": doc_id, "user_id": user["user_id"], "tenant_id": tenant_match(user)},
        {"_id": 0}
    )
    if not doc:
...
async def genera_dichiarazione_cam_pdf(commessa_id: str, user: dict = Depends(get_current_user)):
    ...
    if client_name:
        cliente = await db.clients.find_one(
            {"business_name": client_name, "user_id": user["user_id"], "tenant_id": tenant_match(user)}, {"_id": 0}
        )
...
async def report_aziendale_cam(
    ...
    if commesse_ids:
        comm_cursor = db.commesse.find(
            {"commessa_id": {"$in": commesse_ids}, "user_id": user["user_id"], "tenant_id": tenant_match(user)},
...
async def assegna_archivio_a_commessa(
    ...
    item = await db.archivio_certificati.find_one(
        {"numero_colata": numero_colata, "user_id": user["user_id"], "tenant_id": tenant_match(user)}, {"_id": 0}
    )
    if not item:
    ...
    await db.archivio_certificati.delete_one(
        {"numero_colata": numero_colata, "user_id": user["user_id"], "tenant_id": tenant_match(user)}
    )
...
async def report_cam_mensile_pdf(user: dict = Depends(get_current_user)):
    ...
    batches = await db.material_batches.find(
        {"user_id": uid, "tenant_id": tenant_match(user), "percentuale_riciclato": {"$ne": None}},
