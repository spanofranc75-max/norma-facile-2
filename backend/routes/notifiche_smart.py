...
@router.get("/count")
async def api_count_unread(user: dict = Depends(require_role("admin", "amministrazione", "ufficio_tecnico", "officina"))):
    count = await count_unread(user["user_id"])
    return {"unread": count}


@router.post("/{notification_id}/read")
async def api_mark_read(notification_id: str, user: dict = Depends(require_role("admin", "amministrazione", "ufficio_tecnico", "officina"))):
    ok = await mark_read(notification_id, user["user_id"])
    return {"marked": ok}


@router.post("/read-all")
async def api_mark_all_read(user: dict = Depends(require_role("admin", "amministrazione", "ufficio_tecnico", "officina"))):
    count = await mark_all_read(user["user_id"])
    return {"marked": count}


@router.post("/{notification_id}/archive")
async def api_archive(notification_id: str, user: dict = Depends(require_role("admin", "amministrazione", "ufficio_tecnico", "officina"))):
