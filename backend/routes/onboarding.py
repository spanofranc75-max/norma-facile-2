...
async def _detect_steps(user_id: str) -> dict:
    ...
    has_preventivo = await db.preventivi.count_documents({"user_id": user_id}) > 0
    has_commessa = await db.commesse.count_documents({"user_id": user_id}) > 0
...
async def get_onboarding_status(user: dict = Depends(get_current_user)):
    ...
    # Auto-detect steps
    steps = await _detect_steps(user_id)
