...
async def get_welder(welder_id: str, user: dict = Depends(get_current_user)):
    doc = await db.welders.find_one({"welder_id": welder_id}, {"_id": 0})
    if not doc:
...
async def update_welder(welder_id: str, payload: WelderCreate, user: dict = Depends(get_current_user)):
    existing = await db.welders.find_one({"welder_id": welder_id}, {"_id": 0})
    if not existing:
        raise HTTPException(404, "Saldatore non trovato")
    ...
    await db.welders.update_one({"welder_id": welder_id}, {"$set": update})
    updated = await db.welders.find_one({"welder_id": welder_id}, {"_id": 0})
...
async def delete_welder(welder_id: str, user: dict = Depends(get_current_user)):
    ...
    await db.welders.delete_one({"welder_id": welder_id})
