...
- **BUG FIX: Lotti CAM non eliminabili** - Mancava completamente l'endpoint DELETE per lotti CAM e la cascade delete da material_batches a lotti_cam. Aggiunto: `DELETE /api/cam/lotti/{id}` (singolo), `DELETE /api/cam/lotti/commessa/{id}` (tutti), cascade in `fpc.py` delete_batch. Frontend: pulsante cestino su ogni lotto + pulsante "Elimina Tutti".
